...
def generate_launch_description():
    ...
    map_dir = os.path.join(get_package_share_directory('amr'), 'map', 'factory_map_copy_1.yaml')

    # Names and poses of the robots
    robots = [
        {'name': 'amr_0', 'x_pose': 0.0, 'y_pose': 0.0, 'z_pose': 0.01,
                           'roll': 0.0, 'pitch': 0.0, 'yaw': 3.14},
        {'name': 'amr_1', 'x_pose': 0.0, 'y_pose': 4.0, 'z_pose': 0.01,
                           'roll': 0.0, 'pitch': 0.0, 'yaw': 0.0},
        {'name': 'amr_2', 'x_pose': 12.0, 'y_pose': 4.0, 'z_pose': 0.01,
                           'roll': 0.0, 'pitch': 0.0, 'yaw': 4.71},
        {'name': 'amr_3', 'x_pose': 14.0, 'y_pose': -5.0, 'z_pose': 0.01,
                           'roll': 0.0, 'pitch': 0.0, 'yaw': 0.0},]
    ...
    declare_world_cmd = DeclareLaunchArgument(
        'world',
        default_value=os.path.join(gazebo_dir, 'worlds', 'factory.world'),
    ...
    declare_robot1_params_file_cmd = DeclareLaunchArgument(
        'amr_0_params_file',
        default_value=os.path.join(param_dir, 'nav2_multirobot_params_1.yaml'),
        description='Full path to the ROS2 parameters file to use for robot1 launched nodes')

    declare_robot2_params_file_cmd = DeclareLaunchArgument(
        'amr_1_params_file',
        default_value=os.path.join(param_dir, 'nav2_multirobot_params_2.yaml'),
        description='Full path to the ROS2 parameters file to use for robot2 launched nodes')
    
    declare_robot3_params_file_cmd = DeclareLaunchArgument(
        'amr_2_params_file',
        default_value=os.path.join(param_dir, 'nav2_multirobot_params_3.yaml'),
    ...
    declare_robot4_params_file_cmd = DeclareLaunchArgument(
        'amr_3_params_file',
        default_value=os.path.join(param_dir, 'nav2_multirobot_params_4.yaml'),
