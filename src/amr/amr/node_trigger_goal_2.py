...
class StatusPublishingNavigator2(Node):

    def __init__(self):
        super().__init__('node_trigger_goal_2')

        self._action_client = ActionClient(self, NavigateToPose, '/amr_1/navigate_to_pose')

        self.suscription1 = self.create_subscription(Bool, '/amr_1/start_navigation', self.trigger_callback, 10)

        self.suscription2 = self.create_subscription(String, '/amr_1/pose_x', self.pose_x_callback, 10)

        self.suscription3 = self.create_subscription(String, '/amr_1/pose_y', self.pose_y_callback, 10)

        self.suscription4 = self.create_subscription(String, '/amr_1/pose_yaw', self.pose_yaw_callback, 10)

        self.status_publisher = self.create_publisher(String, '/amr_1/navigation_status', 10)
...
def main(args=None):
    ...
    node = StatusPublishingNavigator2()
    rclpy.spin(node)
    rclpy.shutdown()
